...
/**
 * Random Forest Decision Tree Implementation 
 * @author genia 
 *
 */
public class RandomForestClassificationTree {
	
	// Root node of DecisionTree
	private DecisionTreeNode rootNode;
	private double attributeSubsetSize;
	 ...
	 */
	public RandomForestClassificationTree (ArrayList<ArrayList<Integer>> data, 
			Integer targetAttributeValue,
			ArrayList<Integer> attributeIndices) {
		
		// Compute attribute subset size for random split subsets
		this.attributeSubsetSize = Math.ceil(Math.sqrt(attributeIndices.size()));
	...
	public int majorityValue (ArrayList<Integer> targetAttributeValues) {
		...
		// Compute majority value and its frequency
		int majority = Collections.max(tracker.values());
		
		// Find all values with majority status
		ArrayList<Integer> tiebreaker = new ArrayList<Integer>();
		for (Entry<Integer, Integer> entry : tracker.entrySet()) {
			if (entry.getValue() == majority)
				tiebreaker.add(entry.getKey());
		}
		
		// If multiple values have the majority value count, shuffle the tiebreaker list
		if (tiebreaker.size() > 1) {
			// Randomly shuffle list of tied values
			Collections.shuffle(tiebreaker);
		}
		
		// Return the first value of the tiebreaker list
		return tiebreaker.get(0);
	}
	...
	public DecisionTreeNode buildDecisionTree(ArrayList<ArrayList<Integer>> data, 
		...
		// Construction of decision tree
			...
			// Select random m-size subset of attribute list
			Random rg = new Random();
			ArrayList<Integer> attributeSubset = new ArrayList<Integer>();
			while (attributeSubset.size() <= this.attributeSubsetSize) {
				int next = rg.nextInt(attributeList.size());
				if (!attributeSubset.contains(attributeList.get(next))) {
					attributeSubset.add(attributeList.get(next));
				}
			}
			
			// Select best attribute upon which to classify
			Integer best = this.chooseSplitAttribute(data, targetAttribute, attributeSubset);
